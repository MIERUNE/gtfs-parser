...
def is_coordinate_close(ref_coord, tgt_coord):
    e = 0.000001
    return (
        ref_coord[0] - e <= tgt_coord[0] <= ref_coord[0] + e
...
def is_geometry_close(ref, tgt):
    ...
    if geom_type != tgt["type"]:
        ...
        return all(
            is_coordinate_close(r, t)
            for r, t in zip(ref["coordinates"], tgt["coordinates"])
        )
    ...
    assert feature_exists(
        {
            ...
            "geometry": {
                "type": "LineString",
                "coordinates": [
                    (143.203641761905, 42.9181203333334),
                    (143.202304, 42.919629),
                ],
            },
        },
        route_features,
    ...
    assert feature_exists(
        {
            ...
            "geometry": {
                "type": "LineString",
                "coordinates": [
                    (143.202401000868, 42.9208138405135),
                    (143.203424111165, 42.9183318330811),
                ],
            },
        },
        route_features,
    ...
    assert properties_exists(
        {
            ...
            "geometry": {"type": "Point", "coordinates": [143.1966675, 42.9692865]},
        },
        stop_features,
    )

    route_features = aggregator.read_route_frequency()
    assert len(route_features) == 931
...
def test_unify_delimiter_night(gtfs):
    ...
    aggregator = Aggregator(
        gtfs, yyyymmdd="20210721", delimiter="_", begin_time="200000", end_time="270000"
    )
